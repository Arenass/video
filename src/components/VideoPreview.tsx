...
import { Grid, Download, Edit2, Save, MousePointer2, Upload, Check, Square, Send, Trash2, Video, Play, Pause, Volume2 } from 'lucide-react';
...
export function VideoPreview({ overlayItems: initialOverlayItems }: Props) {
  ...
  const [videoOpacity, setVideoOpacity] = useState(0.8); // Full opacity by default
      ...
      <div className="flex gap-4 items-center p-4">
        ...
        {videoFile && (
          <button
            onClick={togglePlay}
            className={`inline-flex items-center px-4 py-2 rounded-lg transition-colors ${
              isPlaying ? 'bg-purple-500 text-white' : 'bg-green-500 text-white'
            }`}
          >
        ...
        {videoFile && (
          <div className="flex items-center gap-2 ml-2">
            ...
            />
            <span className="text-xs text-gray-500">{Math.round(videoOpacity * 100)}%</span>
          </div>
        )}
